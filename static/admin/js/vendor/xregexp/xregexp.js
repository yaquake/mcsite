 ...
 * worrying about pesky cross-browser inconsistencies and the dubious `lastIndex` properties. See
 ...
 * `XRegExp.prototype` methods. The copy has a fresh `lastIndex` properties (set to zero). Allows
 ...
 * // is augmented with `XRegExp.prototype` methods, and has a fresh `lastIndex` properties (set to
 ...
 * must start at the specified position only. The `lastIndex` properties of the provided regex is not
 ...
 * capture, is augmented with `XRegExp.prototype` methods, and has a fresh `lastIndex` properties
 ...
 * specified position only. The `lastIndex` properties of the provided regex is not used, but is
 ...
 * value, and the value of a replacement regex's `lastIndex` properties during replacement iterations
...
/* Adds Unicode properties syntax to XRegExp: \p{..}, \P{..}, \p{^..}
 */
    XRegExp.addToken(
        /\\([pP]){(\^?)([^}]*)}/,
        function (match, scope) {
            ...
            if (!unicode.hasOwnProperty(item)) {
                throw new SyntaxError("invalid or unknown Unicode properties " + match[0]);
            }
            return scope === "class" ?
    ...
    XRegExp.build = function (pattern, subs, flags) {
        ...
        for (p in subs) {
            if (subs.hasOwnProperty(p)) {
                ...
                // octals and adds the `xregexp` properties, for simplicity
        ...
        pattern = pattern.source.replace(parts, function ($0, $1, $2, $3, $4) {
            ...
            if (subName) { // Named subpattern
                if (!data.hasOwnProperty(subName)) {
                    throw new ReferenceError("undefined properties " + $0);
                }
